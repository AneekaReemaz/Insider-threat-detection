...
try:
    ...
    event_type_encoder = {label: index for index, label in enumerate(pd.factorize(df_full['event_type'].fillna('missing'))[1])}
...
def prepare_features(df_new):
    ...
    df_new['event_type_encoded'] = df_new['event_type'].fillna('missing').map(event_type_encoder).fillna(-1).astype(int)
    return df_new
...
async def predict(request: Request):
    ...
    # This threshold was determined during our model evaluation to be effective
    if loss[0] > 0.01:
         alert = {
